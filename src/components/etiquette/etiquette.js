document.addEventListener('DOMContentLoaded', () => {
  ...
  function getCardWidthWithGap() {
    const card = cardContainer.querySelector('.etiquette__card');
    if (!card) return 0;
  ...
  function renderCards(country) {
    ...
    cardData[country].forEach(card => {
      ...
      imgSpan.style.backgroundImage = `url(../../assets/images/etiquette/${card.image})`;
      ...
      if (Array.isArray(card.desc)) {
        ...
        p.classList.add('etiquette__card-desc');
        p.innerText = card.desc.join('\n');
